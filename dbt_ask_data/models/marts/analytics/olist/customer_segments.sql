...
rfm_scores as (
    ...
    select
        ...
        ntile(5) over (order by total_order_count desc) as f_score,

        -- Monetary Score: Higher total_revenue_generated is better (higher score)
        ntile(5) over (order by life_time_customer_value desc) as m_score
...
rfm_segments as (
    ...
    select
        ...
        f_score,
        m_score,
        cast(r_score as text) || cast(f_score as text) || cast(m_score as text) as rfm_segment_score
    from rfm_scores
)
