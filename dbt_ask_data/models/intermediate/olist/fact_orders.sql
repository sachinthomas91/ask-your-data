...
,aggregate_order_lines as (
  select 
    ...
    ,round(sum(item_price)::numeric, 2) as order_item_price_total
    ,round(sum(item_freight_value)::numeric, 2) as order_freight_total
    ,round((sum(item_price) + sum(item_freight_value))::numeric, 2) as order_total
  from
    order_lines
  group by 
    order_id
...
select
  ...
  ,EXTRACT(EPOCH FROM (ord.order_delivered_customer_datetime - ord.order_estimated_delivery_datetime)) / 3600 as delivery_estimation_slip_hours
